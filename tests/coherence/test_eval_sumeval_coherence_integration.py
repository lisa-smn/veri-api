"""
Mini-Integration-Test für eval_sumeval_coherence.py.
"""
...
def mini_fixture_path():
    ...
    fixture_path = Path(__file__).parent.parent / "fixtures" / "sumeval_coherence_mini.jsonl"
...
def test_eval_sumeval_coherence_importable(monkeypatch):
    ...
    with unittest.mock.patch("dotenv.load_dotenv"):
        ...
        try:
            # Mock auch OpenAIClient, um Import-Probleme zu vermeiden
            with unittest.mock.patch("app.llm.openai_client.OpenAIClient"):
                import eval_sumeval_coherence

                assert hasattr(eval_sumeval_coherence, "main")
        except (ImportError, PermissionError) as e:
            pytest.skip(f"Script nicht importierbar: {e}")


def test_eval_sumeval_coherence_output_structure(mini_fixture_path, temp_output_dir, monkeypatch):
    """
    ...
    script_path = Path(__file__).parent.parent.parent / "scripts" / "eval_sumeval_coherence.py"
    if not script_path.exists():
        pytest.skip(reason=f"Script nicht gefunden: {script_path}")

    # Prüfe, ob wir in CI sind (dann skip mit konsistenter Begründung)
    skip_reason = "Subprocess execution restricted in CI sandbox"
    if os.getenv("CI") == "true":
        pytest.skip(reason=skip_reason)

    # Lokal: Versuche Subprocess-Ausführung
    # Mock dotenv in Subprocess (nicht möglich, daher skip wenn dotenv fehlschlägt)
    try:
        ...
        # Wenn dotenv fehlschlägt (PermissionError), ist das ok - skip
...
def test_mini_fixture_format(mini_fixture_path):
    ...
    for i, line in enumerate(lines, 1):
        try:
            ...
            assert "coherence" in data["gt"], f"Zeile {i}: Fehlt 'gt.coherence'"
