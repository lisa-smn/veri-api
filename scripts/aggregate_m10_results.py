...
def aggregate_results(
    ...
    for result in run_results:
        ...
        row = {
            ...
            "config_name": config.get("description", "").split(" - ")[0] if config.get("description") else "",
            "n": result["n"],
...
def generate_summary_md(
    ...
    if frank_runs:
        ...
        if baseline and tuned:
            ...
            lines.append(f"### FRANK (Dev/Calibration)")
            lines.append("")
            lines.append(f"- **Baseline Balanced Acc:** {baseline['balanced_accuracy']:.3f} | F1: {baseline['f1']:.3f}")
            lines.append(f"- **Tuned Balanced Acc:** {tuned['balanced_accuracy']:.3f} ({bal_acc_improvement:+.3f}) | F1: {tuned['f1']:.3f} ({f1_improvement:+.3f})")
            if ablation:
                ablation_drop_bal = baseline["balanced_accuracy"] - ablation["balanced_accuracy"]
                ablation_drop_f1 = baseline["f1"] - ablation["f1"]
                lines.append(f"- **Ablation Balanced Acc:** {ablation['balanced_accuracy']:.3f} ({ablation_drop_bal:+.3f} vs baseline) | F1: {ablation['f1']:.3f} ({ablation_drop_f1:+.3f})")
                lines.append(f"  → Ablation zeigt {'deutlichen' if ablation_drop_bal > 0.05 else 'geringen'} Effekt der Claim-Extraktion")
            lines.append("")
    ...
    if finesumfact_runs:
        ...
        if final:
            lines.append(f"### FineSumFact (Test)")
            lines.append("")
            lines.append(f"- **Final Balanced Acc:** {final['balanced_accuracy']:.3f} | F1: {final['f1']:.3f}")
            if tuned:
                generalization_bal = final["balanced_accuracy"] - tuned["balanced_accuracy"]
                generalization_f1 = final["f1"] - tuned["f1"]
                lines.append(f"- **Generalization:** Balanced Acc {generalization_bal:+.3f} | F1 {generalization_f1:+.3f} vs FRANK Tuned")
                if abs(generalization_bal) < 0.05:
                    lines.append("  → Gute Generalisierung auf FineSumFact (keine Parameteränderungen nach FRANK)")
                elif generalization_bal < 0:
                    lines.append("  → Performance-Drop auf FineSumFact (mögliche Dataset-Differenzen)")
                else:
            ...
            if ablation:
                ...
                lines.append(f"- **Ablation Balanced Acc:** {ablation['balanced_accuracy']:.3f} ({ablation_drop_bal:+.3f} vs final) | F1: {ablation['f1']:.3f} ({ablation_drop_f1:+.3f})")
            lines.append("")
    ...
    if combined_runs:
        ...
        lines.append("")
        lines.append(f"- **Combined Balanced Acc:** {combined['balanced_accuracy']:.3f} | F1: {combined['f1']:.3f}")
    ...
    if tuned:
        lines.append(f"- **Recall vs Specificity:** Recall={tuned['recall']:.3f}, Specificity={tuned['specificity']:.3f}")
        if tuned['recall'] > tuned['specificity']:
            lines.append("  → System priorisiert Recall (findet mehr Fehler, aber auch mehr False Positives)")
        elif tuned['specificity'] > tuned['recall']:
            lines.append("  → System priorisiert Specificity (weniger False Positives, aber möglicherweise mehr übersehene Fehler)")
        else:
    ...
    if frank_runs:
        ...
        if baseline and ablation:
            ...
            lines.append(f"### Ablation-Effekt")
            lines.append("")
            lines.append(f"- **Balanced Acc-Drop durch Ablation:** {ablation_effect_bal:.3f} | F1-Drop: {ablation_effect_f1:.3f}")
            if ablation_effect_bal > 0.1:
                lines.append("  → **Starker Effekt:** Claim-Extraktion trägt erheblich zur Performance bei")
