...
def suggest_tuned_config(baseline_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    examples_path = ROOT / "results" / "evaluation" / "runs" / "results" / f"{baseline_results['run_id']}_examples.jsonl"
    examples = []
    ...
    if fp > fn * 1.5:
        ...
        suggestions["suggested_error_threshold"] = max(1, baseline_results["config"].get("error_threshold", 1) + 1)
    ...
    if examples:
        ...
        if high_conf_errors > len(examples) * 0.1:
            suggestions["notes"].append("Viele high-confidence Errors → Score-Cutoff in Betracht ziehen")
...
def main():
    ...
    print(f"   Set factuality_frank_tuned_v1.error_threshold = {suggestions['suggested_error_threshold']}")
