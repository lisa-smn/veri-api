...
def load_examples(path: Path, max_examples: Optional[int] = None) -> List[Dict[str, Any]]:
...
def run_single_evaluation(
    ...
    if run_config["dataset"] == "finesumfact":
        cache_path = ROOT / "results" / "evaluation" / "factuality" / f"cache_finesumfact_{run_config['llm_model']}_{prompt_version}.jsonl"
    else:
        cache_path = ROOT / "results" / "evaluation" / "factuality" / f"cache_{run_config['llm_model']}_{prompt_version}.jsonl"
    
    cache = load_cache(cache_path) if cache_enabled else {}
    cache_hits = 0
    cache_misses = 0
    
    logger.info(f"Cache: {len(cache)} entries loaded from {cache_path.name if cache_enabled else 'disabled'}")
    ...
    decision_threshold_float = run_config.get("decision_threshold_float")  # Neu: float threshold für gewichtete Aggregation
    ...
    def compute_weighted_issue_score(issue_spans, uncertainty_policy: str, severity_weights: dict, confidence_min: float) -> float:
        """
        ...
        for span in issue_spans:
            ...
            # Uncertainty weight
            is_uncertain = "uncertain" in (span.message or "").lower() or "nicht sicher verifizierbar" in (span.message or "").lower()
        ...
        try:
            ...
            if cached_result:
                ...
                agent_result = agent.run(
                    ex["article"],
                    ex["summary"],
                    meta={"source": "m10_evaluation", "run_id": run_id, "example_id": ex["example_id"]},
                )
            ...
            if decision_threshold_float is not None:
                ...
                if decision_mode == "issues":
                    ...
                    pred_has_error = (score_cutoff is not None) and (agent_result.score < score_cutoff)
            ...
            weighted_score = compute_weighted_issue_score(
                ...
                severity_weights,
                confidence_min
            ) if decision_threshold_float is not None else None
            
            effective_issues = count_effective_issues(agent_result.issue_spans, uncertainty_policy) if decision_threshold_float is None else None
    ...
    if cache_enabled:
        logger.info(f"Cache: {cache_hits} hits, {cache_misses} misses ({cache_hits/(cache_hits+cache_misses)*100:.1f}% hit rate)")
...
def analyze_failure_patterns(examples: List[Dict[str, Any]]) -> str:
    ...
    if fp_examples:
        ...
        if fp_issue_types:
            ...
            lines.append(f"  - Häufigste Issue-Types: {', '.join(f'{t}({c})' for t, c in top_types)}")
        lines.append("")
...
def generate_run_documentation(
    ...
    fp_examples.sort(key=lambda x: x.get("score", 1.0), reverse=True)  # Highest scores = most confident FPs
    ...
    if fp_examples:
        ...
        for i, ex in enumerate(fp_examples[:5], 1):
            ...
            summary_snippet = (summary_text[:100] + "...") if len(summary_text) > 100 else summary_text
            lines.append(f"{i}. **ID:** {ex['example_id']} | **Score:** {ex['score']:.3f} | **Issues:** {ex['num_issues']}")
    ...
    if metrics['fp'] > metrics['fn']:
        lines.append("**Dominantes Pattern:** False Positives überwiegen. System ist zu konservativ und markiert korrekte Summaries als fehlerhaft.")
    elif metrics['fn'] > metrics['fp']:
        lines.append("**Dominantes Pattern:** False Negatives überwiegen. System übersieht Fehler in Summaries.")
    else:
...
def save_run_results(
    ...
    examples_path = ROOT / "results" / "evaluation" / "runs" / "results" / f"{run_id}_examples.jsonl"
