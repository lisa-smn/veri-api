...
def load_summary_matrix() -> List[Dict[str, Any]]:
    ...
    with csv_path.open("r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            for key in ["n", "tp", "fp", "tn", "fn", "precision", "recall", "f1", 
                       "specificity", "accuracy", "auroc", "balanced_accuracy"]:
                if key in row:
                    try:
...
def select_best_tuned_run(
    ...
    recall_filtered = [
        r for r in tuning_runs
        if r.get("recall", 0.0) >= recall_min
    ]
    ...
    if specificity_gate_disabled:
        ...
        both_gates_filtered = [
            r for r in recall_filtered
            if r.get("specificity", 0.0) >= specificity_min
        ]
        stats["after_specificity_gate"] = len(both_gates_filtered)
        stats["both_gates_passed"] = len(both_gates_filtered) > 0
    ...
    if stats.get("specificity_gate_disabled", False):
        justification.append(f"⚠️  Gate 2: specificity >= {specificity_min} (DEAKTIVIERT: nur {stats.get('gt_negatives', 0):.0f} GT-Negatives, < 20)")
        ...
        if stats["both_gates_passed"] and candidate_pool_name == "both_gates":
            ...
            justification.append(f"❌ Gate 2: specificity >= {specificity_min} (kein Run erfüllt beide Gates)")
    ...
    justification.append(f"✅ Beste Zielmetrik ({target_metric.upper()}) innerhalb Kandidatenmenge: {best.get(target_metric, 0.0):.3f}")
...
def save_selection_artifacts(
    ...
    if candidate_set:
        ...
        sorted_runs = sorted(candidate_set, key=lambda r: get_num(r, target_metric), reverse=True)[:5]
        
        # CSV schreiben
        with csv_path.open("w", encoding="utf-8", newline="") as f:
            writer = csv.DictWriter(
                f,
                fieldnames=["run_id", "recall", "precision", "specificity", "f1", "mcc", "balanced_accuracy", target_metric],
            )
            writer.writeheader()
            for r in sorted_runs:
                writer.writerow({
...
def get_run_config(run_id: str) -> Optional[Dict[str, Any]]:
...
def main():
    ...
    parser.add_argument("--recall-min", type=float, default=0.90, help="Minimum recall (Gate 1, default: 0.90)")
    parser.add_argument("--specificity-min", type=float, default=0.20, help="Minimum specificity (Gate 2, default: 0.20)")
    parser.add_argument("--target", type=str, choices=["balanced_accuracy", "mcc"], default="mcc", 
                       help="Optimierungsziel (default: mcc)")
    parser.add_argument("--dataset", type=str, default="frank", help="Dataset to filter (default: frank)")
    parser.add_argument("--output-config", type=str, default=None, help="Output path for best config YAML snippet")
    ...
    if stats.get("gt_negatives") is not None:
        ...
        if gt_neg < 20:
            print(f"  ⚠️  Warnung: Nur {gt_neg:.0f} GT-Negatives (< 20) - Specificity-Gate deaktiviert")
        else:
    ...
    print(f"  Specificity: {best['specificity']:.3f} {'✅' if best['specificity'] >= args.specificity_min else '❌'}")
    ...
    if config:
        ...
        yaml_lines.append(f"score_cutoff: {config.get('score_cutoff') if config.get('score_cutoff') is not None else 'null'}")
