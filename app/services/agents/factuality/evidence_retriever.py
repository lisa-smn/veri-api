...
class EvidenceRetriever:
    ...
    def _split_into_passages(self, text: str) -> List[str]:
        ...
        # Filtere leere Sätze
        sentences = [s.strip() for s in sentences if s.strip() and len(s.strip()) >= self.min_passage_length]
    ...
    def _score_passage(self, passage: str, claim_tokens: set, claim_text: str) -> float:
        ...
        numbers_claim = set(re.findall(r'\d{1,4}', claim_text))
        numbers_passage = set(re.findall(r'\d{1,4}', passage))
        ...
        generic_tokens = {"The", "A", "An", "BBC", "Mr", "Mrs", "Dr", "Prof", "Inc", "Ltd", "USA", "UK"}
        ...
        if entities_claim:
            ...
            hit_count = sum(1 for ent in entities_claim if ent.lower() in passage_lower or ent in passage)
            if hit_count > 0:
