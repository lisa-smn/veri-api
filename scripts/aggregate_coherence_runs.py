...
def load_run_metadata(path: Path) -> Optional[Dict[str, Any]]:
...
def determine_system_type(summary: Dict[str, Any], metadata: Optional[Dict[str, Any]]) -> str:
    """Bestimmt System-Typ (agent, rouge_l, bertscore, stress_shuffle, stress_inject)."""
    # Aus metadata
    if metadata:
...
def extract_model_prompt(metadata: Optional[Dict[str, Any]]) -> tuple[str, str]:
...
def write_summary_md(df: pd.DataFrame, out_path: Path) -> None:
    ...
    for system in df["system"].unique():
        ...
        system_df = df[df["system"] == system].sort_values("pearson", ascending=False, na_last=True)
        
        # Header
        lines.append("| Run ID | Model | Prompt | Seed | Pearson r | Spearman ρ | MAE | RMSE | R² | n_used |")
        lines.append("|--------|-------|--------|------|----------|------------|-----|------|-----|--------|")
