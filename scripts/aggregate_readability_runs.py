"""
Aggregiert mehrere Readability-Evaluation-Runs zu einer Vergleichstabelle.

Liest mehrere summary.json (agent + optional baselines/judge) und schreibt:
- summary_matrix.csv (tabellarischer Vergleich)
- summary_matrix.md (human-readable)

Input: Verzeichnisse mit summary.json (z.B. results/evaluation/readability/*/summary.json)
"""
...
def determine_system_type(summary: dict[str, Any], metadata: dict[str, Any] | None) -> str:
    """Bestimmt System-Typ (agent, rouge_l, bertscore, llm_judge)."""
...
def aggregate_runs(run_dirs: list[Path]) -> pd.DataFrame:
    ...
    for run_dir in run_dirs:
        # Skip Dateien (nur Verzeichnisse)
    ...
    if skipped_incomplete:
        print(f"\nÜbersprungene incomplete Runs ({len(skipped_incomplete)}):")
        for run_id in skipped_incomplete:
            print(f"  - {run_id}")
...
def write_summary_md(df: pd.DataFrame, out_path: Path) -> None:
    ...
    lines = [
        "# Readability Evaluation Comparison",
        "",
        "## Summary Matrix",
        "",
    ]
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Aggregiert mehrere Readability-Evaluation-Runs")
    ap.add_argument(
        "run_dirs",
        nargs="+",
        type=str,
        help="Verzeichnisse mit summary.json (z.B. results/evaluation/readability/*)",
    )
    ap.add_argument(
        "--out",
        type=str,
        default="results/evaluation/readability/summary_matrix",
        help="Output-Pfad (ohne Extension, default: results/evaluation/readability/summary_matrix)",
    )

    args = ap.parse_args()

    # Expandiere Wildcards und filtere nur Verzeichnisse
    run_dirs = []
    for pattern in args.run_dirs:
        ...
        for path_str in expanded:
            ...
            if path.is_dir():
                ...
                print(f"Warnung: Überspringe Datei (kein Verzeichnis): {path}")

    # Filtere Verzeichnisse ohne summary.json (mit Warnung)
    valid_dirs = []
    for d in run_dirs:
        if not d.exists():
            print(f"Warnung: Verzeichnis existiert nicht: {d}")
            continue
        if not (d / "summary.json").exists():
            print(f"Warnung: summary.json nicht gefunden in: {d}, überspringe")
            continue
        valid_dirs.append(d)

    if not valid_dirs:
        ap.error("Keine gültigen Run-Verzeichnisse gefunden (mit summary.json)")

    print(f"Aggregiere {len(valid_dirs)} Runs...")
    df = aggregate_runs(valid_dirs)
