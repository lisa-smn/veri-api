#!/bin/bash
# Startet Factuality-Evaluation für Bachelorarbeit
# - 300 Beispiele (repräsentative Stichprobe)
# - Mit Cache (schneller)

cd "$(dirname "$0")"

echo "=========================================="
echo "Factuality Evaluation - Bachelorarbeit"
echo "=========================================="
echo ""

# Prüfe Python
if command -v python3 &> /dev/null; then
    PYTHON_CMD="python3"
elif command -v python &> /dev/null; then
    PYTHON_CMD="python"
else
    echo "Error: Python not found"
    exit 1
fi

echo "Using Python: $PYTHON_CMD"
echo ""

# Config
CONFIG="evaluation_configs/factuality_frank_test_v1.json"
DATASET="data/frank/frank_clean.jsonl"

# Prüfe Config
if [ ! -f "$CONFIG" ]; then
    echo "Error: Config not found: $CONFIG"
    exit 1
fi

# Prüfe Dataset
if [ ! -f "$DATASET" ]; then
    echo "Error: Dataset not found: $DATASET"
    exit 1
fi

echo "Config: $CONFIG"
echo "Dataset: $DATASET"
echo "Examples: 300 (max_examples in config)"
echo "Cache: Enabled"
echo ""

# Prüfe Cache
CACHE="results/evaluation/factuality/cache_gpt-4o-mini_v3_uncertain_spans.jsonl"
if [ -f "$CACHE" ]; then
    CACHE_SIZE=$(wc -l < "$CACHE" 2>/dev/null || echo "0")
    echo "Cache gefunden: $CACHE_SIZE Einträge"
    echo "→ Bereits evaluierte Beispiele werden übersprungen"
else
    echo "Kein Cache vorhanden - alle Beispiele werden neu evaluiert"
fi
echo ""

# Starte Evaluation
echo "Starting evaluation..."
echo ""

$PYTHON_CMD scripts/eval_factuality_structured.py "$CONFIG" --dataset-path "$DATASET"

EXIT_CODE=$?

echo ""
echo "=========================================="
if [ $EXIT_CODE -eq 0 ]; then
    echo "✅ Evaluation completed successfully!"
    echo ""
    echo "📊 Results:"
    echo "  - Documentation: results/evaluation/runs/docs/factuality_frank_test_v1.md"
    echo "  - Metrics: results/evaluation/runs/results/factuality_frank_test_v1.json"
    echo "  - Examples: results/evaluation/runs/results/factuality_frank_test_v1_examples.jsonl"
    echo "  - Analysis: results/evaluation/runs/analyses/factuality_frank_test_v1.json"
    echo ""
    echo "📈 Quick view:"
    echo "  cat results/evaluation/runs/docs/factuality_frank_test_v1.md | head -50"
else
    echo "❌ Evaluation failed with exit code $EXIT_CODE"
    echo "Check the output above for details."
fi
echo "=========================================="

exit $EXIT_CODE

